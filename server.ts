...
function searchMatches(): boolean {
  ...
  // STEP 3: Last resort - just match any two users
...
SERVER.on('upgrade', (request, socket, head) => {
  ...
  // Only handle VibeChat websocket connections, ignore Vite HMR
  if (!urlParams.pathname.startsWith('/api/ws')) {
    return; // Let other handlers (like Vite) process this
  }
