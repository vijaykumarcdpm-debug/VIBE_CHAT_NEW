...
export default function App() {
  ...
  return (
    <div id="app-root-container" className={`h-[100dvh] w-full max-w-[100vw] overflow-hidden font-sans antialiased flex flex-col justify-between theme-transition ${
      ...
      {wallpaper && screen === 'lobby' && sidebarTab === 'chat' && (
          <div 
              className="fixed inset-0 z-0 bg-cover bg-center bg-no-repeat transition-opacity duration-300 pointer-events-none"
              style={{ 
              ...
              onError={() => {
                  if (wallpaper) failedWallpapers.add(wallpaper);
                  setWallpaper(null);
              }}
          />
      )}

      <div className="flex-1 flex flex-col w-full max-w-[100vw] h-full min-h-0 overflow-hidden relative z-10">
      ...
      {activeCall && (
        <div className="fixed inset-0 bg-[#070B16] z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-4xl [&_video]:!scale-x-100 [&_video]:!-webkit-scale-x-100">
            <AudioVideoCall
              ws={ws}
              userId={me?.id || ''}
          ...
          <div className={`w-full max-w-md max-h-[95vh] overflow-y-auto scrollbar-thin border rounded-3xl p-8 space-y-6 shadow-2xl relative text-center ${theme === 'light' ? 'bg-white border-slate-200 text-slate-800' : 'bg-slate-900 border-slate-800 text-slate-100'}`}>
            ...
            <div className={`p-4 rounded-2xl text-left space-y-3 pt-3 text-xs ${theme === 'light' ? 'bg-slate-50 border border-slate-200 text-slate-700' : 'bg-slate-950/60 border border-slate-800 text-slate-300'}`}>
              {["1. Be respectful and kind to all members.", "2. No inappropriate content in profiles.", "3. No spam or promotions."].map((rule, idx) => (
                <div key={idx} className="flex gap-2"><span className={`text-[13px] ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>{rule}</span></div>
              ))}
            </div>
      ...
      {adminChangesNotice && (
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-md z-[160] flex items-center justify-center p-4 animate-fade-in text-slate-100">
          <div className={`w-full max-w-sm border rounded-3xl p-6 relative overflow-hidden shadow-2xl max-h-[82vh] overflow-y-auto scrollbar-thin ${theme === 'light' ? 'bg-white border-slate-200 text-slate-900 shadow-xl' : 'bg-slate-900 border-slate-800 text-slate-100'}`}>
            ...
            <div className="flex flex-col items-center text-center space-y-4">
              <h3 className={`text-lg font-black font-display tracking-tight ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>Profile Updated by Admin</h3>
              <div className={`w-full border rounded-2xl p-4 text-left space-y-2 text-xs overflow-y-auto max-h-40 scrollbar-thin ${theme === 'light' ? 'bg-slate-50 border-slate-250 text-slate-700' : 'bg-slate-950/50 border-slate-800'}`}>
                {adminChangesNotice.changes.map((change, idx) => (
                  <div key={idx} className="flex items-start gap-2"><span className="text-emerald-500">✔</span><span className={`font-semibold ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>{change}</span></div>
                ))}
              </div>
      ...
      {showOwnProfileModal && me && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[110] flex items-center justify-center p-4 animate-fade-in text-slate-100">
          <div className={`w-full max-w-lg overflow-y-auto scrollbar-thin rounded-3xl p-6 relative shadow-2xl border max-h-[82vh] ${
            ...
            <form onSubmit={handleSaveProfile} className="space-y-3 font-display">
              ...
              <div className="flex flex-col items-center gap-1">
                ...
                <label className={`px-2 py-0.5 text-[9px] font-bold border rounded-lg transition cursor-pointer ${theme === 'light' ? 'text-slate-900 bg-slate-100' : 'text-white bg-slate-800'}`}>Change picture<input type="file" accept="image/png,image/jpeg" onChange={handleProfilePicUpload} className="hidden" /></label>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                <div>
                  <label className={`block text-[9px] uppercase font-extrabold tracking-wider mb-0.5 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>Display Username</label>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className={`block text-[9px] uppercase font-extrabold tracking-wider mb-0.5 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>Gender</label>
                  ...
                  <div>
                    <label className={`block text-[9px] uppercase font-extrabold tracking-wider mb-0.5 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>Age</label>
                ...
                <div className="sm:col-span-2">
                  <label className={`block text-[9px] uppercase font-extrabold tracking-wider mb-0.5 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>About Companion / Bio (Max 50 Words)</label>
                  <textarea rows={2} value={editBio} onChange={(e) => setEditBio(e.target.value)} placeholder="Type bio details..." className={`w-full text-xs p-2 rounded-xl outline-none border transition resize-none whitespace-normal break-words ${theme === 'light' ? 'bg-slate-50 border-slate-200 text-slate-900' : 'bg-slate-950 border-slate-800 text-white'}`} />
                </div>
                <div>
                  <label className={`block text-[9px] uppercase font-extrabold tracking-wider mb-0.5 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>City</label>
                ...
                <div>
                  <label className={`block text-[9px] uppercase font-extrabold tracking-wider mb-0.5 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>State</label>
                ...
                <div className="sm:col-span-2">
                  <label className={`block text-[9px] uppercase font-extrabold tracking-wider mb-0.5 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>Country</label>
                  <input type="text" value={editCountry} onChange={(e) => setEditCountry(e.target.value)} disabled={!isUserVIP} className={`w-full text-xs p-2 rounded-xl outline-none border transition ${theme === 'light' ? 'bg-slate-50 border-slate-200' : 'bg-slate-950 border-slate-800'}`} />
                </div>
              </div>
              <div className="space-y-1.5 pt-2 border-t border-slate-500/10">
                <div className="flex gap-2">
                  <button type="submit" className={`w-1/2 py-2 text-xs font-bold text-white rounded-xl shadow cursor-pointer transition hover:scale-[1.01] ${theme === 'light' ? 'bg-blue-600 hover:bg-blue-500' : 'bg-violet-600 hover:bg-violet-500'}`}>Save Changes</button>
                  <button type="button" onClick={() => setShowOwnProfileModal(false)} className={`w-1/2 py-2 text-xs font-semibold rounded-xl border cursor-pointer ${theme === 'light' ? 'bg-slate-100 text-slate-900' : 'bg-slate-800 text-white'}`}>Back To Lobby</button>
                </div>
              </div>
            </form>
          </div>
      ...
      {token && !me ? (
        <div className={`min-h-screen flex flex-col items-center justify-center font-display w-full max-w-full ${theme === 'light' ? 'bg-slate-50' : 'bg-slate-950'}`}>
          <div className="relative flex flex-col items-center max-w-sm px-6 text-center">
            ...
            <h3 className={`text-xl font-black tracking-tight mb-2 uppercase ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>Verifying Gateway Access</h3>
            <p className="text-xs text-slate-400 leading-relaxed">Authenticating credentials with lounge nodes... Please wait.</p>
            <button onClick={() => { localStorage.clear(); setToken(null); setMe(null); }} className={`mt-8 px-4 py-2 border rounded-xl text-xs font-bold uppercase cursor-pointer ${theme === 'light' ? 'bg-white text-slate-900' : 'bg-slate-900 text-white'}`}>Cancel & Start Fresh</button>
          </div>
        </div>
      ) : !token || !me ? (
        isAdminPortal ? (
          ...
          <header className={`border-b border-opacity-50 relative z-20 w-full ${theme === 'light' ? 'bg-white border-slate-200 shadow-sm text-slate-800' : 'bg-slate-900/80 border-slate-800 backdrop-blur-md text-stone-100'}`}>
            <div className="max-w-7xl mx-auto px-3 sm:px-6 h-16 sm:h-20 flex items-center justify-between w-full">
              <button onClick={() => { setScreen('lobby'); setSidebarTab('people'); fetchStats(); }} className="flex items-center gap-2 sm:gap-3 cursor-pointer text-left focus:outline-none shrink-0">
                ...
                <div className="block">
                  <span className={`text-lg sm:text-xl font-bold font-display tracking-tight block leading-none ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>VibeChat</span>
              ...
              <div className="flex items-center gap-1.5 sm:gap-3 relative shrink-0">
                <div className="relative">
                  <button onClick={() => setShowNotificationsDropdown(!showNotificationsDropdown)} className={`p-2.5 rounded-xl border relative cursor-pointer ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}><Bell className="w-4 h-4" /></button>
                </div>

                <button onClick={() => { const next = theme === 'light' ? 'dark' : 'light'; setTheme(next); localStorage.setItem('vibechat_theme', next); }} className={`p-2.5 rounded-xl border cursor-pointer ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>{theme === 'light' ? <Moon className="w-4 h-4" /> : <Sun className="w-4 h-4" />}</button>

                <button onClick={() => setShowOwnProfileModal(true)} className="p-1.5 sm:pr-3 rounded-xl border flex items-center gap-2 max-w-[155px] cursor-pointer">
                  ...
                  <span className={`hidden sm:flex text-xs font-bold truncate uppercase max-w-[75px] items-center gap-1 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>
                    {me.type === 'Admin' ? 'ADMIN' : me.username}
                    {me.type === 'Royal VIP' && <span className="text-amber-400">👑</span>}
                    {me.type === 'Moderator' && <ShieldCheck className="w-4 h-4 inline-block text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.75)] shrink-0 align-middle animate-pulse" />}
                  </span>
                </button>

                <button onClick={handleLogout} className="p-2 sm:p-2.5 rounded-xl border bg-rose-50 text-rose-600 cursor-pointer"><LogOut className="w-4 h-4" /></button>
              </div>
            </div>
          </header>
          ...
          {announcement && (
            <div className={`border-b w-full max-w-full ${theme === 'light' ? 'bg-slate-50 border-slate-200' : 'bg-gradient-to-r from-violet-950/40 border-violet-500/20'}`}>
              <div className="max-w-7xl mx-auto px-4 py-2.5 overflow-hidden flex items-center gap-3 w-full">
                ...
                <div className={`relative flex overflow-hidden text-xs select-none font-semibold flex-1 min-w-0 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>
              ...
              <div className={`w-full max-w-full flex flex-col flex-1 min-h-0 sm:rounded-[2rem] overflow-hidden shadow-2xl border ${theme === 'light' ? 'bg-white/95 border-slate-200 shadow-indigo-500/10' : 'bg-slate-950/95 border-violet-500/20 shadow-violet-500/20'}`}>
                {me.photoVerificationPending ? (
                  <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                    ...
                    <h2 className={`text-2xl font-bold font-display ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>Verification in Progress</h2>
      ...
      {showExitConfirm && (
        <div className={`fixed inset-0 backdrop-blur-md z-[200] flex items-center justify-center p-4 text-center animate-fade-in ${theme === 'light' ? 'bg-slate-900/40 text-slate-900' : 'bg-slate-950/80 text-slate-100'}`}>
          <div className={`p-8 border rounded-3xl max-w-sm w-full space-y-6 shadow-2xl relative overflow-hidden ${theme === 'light' ? 'bg-white border-slate-200' : 'bg-slate-900 border-slate-800'}`}>
            ...
            <h3 className={`text-xl font-bold font-display tracking-tight relative z-10 ${theme === 'light' ? 'text-slate-900' : 'text-white'}`}>Exit WeChat Connection?</h3>
