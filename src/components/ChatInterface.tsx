...
interface ChatInterfaceProps {
  ...
  onInitiateCall: (recipientId: string, type: 'audio' | 'video', name?: string, pic?: string) => void;
  ...
  onNewMessageNotification?: (msgText: string) => void;
}
...
export default function ChatInterface({
  ...
  const inspectedIsOnline = Boolean(
    isUserOnline(inspectedPeer?.id || inspectedFullDetails?.id)
  );
  ...
  const fetchSideData = async (force: boolean = false) => {
    ...
    try {
      ...
      if (mountedRef.current) {
        setOnlineUsers(onlineData.map((u: any) => ({ ...u, online: true })));
      }
    } catch (e) {
    ...
    const handleMessages = (e: MessageEvent) => {
      try {
        ...
        if (event === 'match:searching') {
          ...
          clearMatchSearchTimeout();
          setMatchedPeerName(data.peerName);
          setIsMatchingLoading(false);
          setShowNoMatchPopup(false);
          setShowNoOnlineUsersPopup(false);
          setShowMatchErrorPopup(false);
          ...
          if (activePartner && activePartner.id === incomingMsg.senderId) {
            const isCurrentlyViewing = focusRoomActiveRef.current;
            setHistoryMessages(prev => [...prev, { ...incomingMsg, read: isCurrentlyViewing }]);
            
            if (isCurrentlyViewing) {
              clearUnreadForPeer(incomingMsg.senderId, true);
            } else {
              // Not actively viewing, so show notification and update unread count
              fetchSideData(true);
              const previewText = incomingMsg.type === 'text' ? incomingMsg.content : `[${incomingMsg.type}]`;
              onNewMessageNotification?.(previewText);
  ...
  // Handle next match and call ended events from App component
  useEffect(() => {
    ...
    window.addEventListener('app:call_ended', handleCallEndedEvent);
    return () => {
      window.removeEventListener('app:next_match', handleNextMatchEvent);
      window.removeEventListener('app:call_ended', handleCallEndedEvent);
    };
  });
  ...
  const handleStartMatching = () => {
    ...
    setSearchStatusIndex(0);
    setChatState('searching');
    // Removed 60-second timeout to allow infinite matching.
  };
  ...
  let leftSidebarClasses = '';
  let rightPaneClasses = '';

  if (sidebarTab === 'chat') {
    leftSidebarClasses = !activePartner 
      ? 'flex w-full md:w-[420px] lg:w-[500px]' 
      : 'hidden md:flex md:w-[420px] lg:w-[500px]';
    rightPaneClasses = !!activePartner 
      ? 'flex w-full md:w-auto' 
      : 'hidden md:flex md:w-auto';
  } else if (sidebarTab === 'lounge') {
    leftSidebarClasses = !activePartner ? 'flex w-full' : 'hidden';
    rightPaneClasses = !!activePartner ? 'flex w-full' : 'hidden';
  } else {
    // 'people' or 'vip'
    leftSidebarClasses = 'flex w-full';
    rightPaneClasses = 'hidden';
  }

  return (
    <div className="flex h-full flex-1 min-h-0 w-full bg-transparent">
      
      {/* SIDE BAR LAYOUT: RECENT CHATS & FRIENDS LIST */}
      <div className={`${leftSidebarClasses} shrink-0 flex flex-col min-h-0 flex-1 transition-all duration-300 ${
        theme === 'light'
...
<span className={`text-sm truncate block font-medium max-w-[220px] ${theme === 'light' ? 'text-slate-600' : 'text-slate-400'}`}>{rc.lastMessage}</span>
        ...
        {/* MAIN CHAT PANE */}
        <div className={`${rightPaneClasses} flex-1 flex-col justify-between min-h-0 relative min-w-0`}>
          ...
          <div className={`p-4 sm:p-5 md:p-6 lg:py-5 lg:px-8 border-b flex justify-between items-center relative z-[80] gap-4 shrink-0 transition duration-300 flex-nowrap ${
            ...
            <div className="flex items-center gap-2">
              {activePartner && (
                <>
                  <button
                    onClick={() => onInitiateCall(activePartner.id!, 'audio', activePartner.username || 'Stranger', activePartner.profilePic || '')}
                  ...
                  <button
                    onClick={() => onInitiateCall(activePartner.id!, 'video', activePartner.username || 'Stranger', activePartner.profilePic || '')}
                    className={`p-2 border rounded-xl transition cursor-pointer ${
                      theme === 'light'
                        ? 'bg-slate-100 hover:bg-slate-200 border-slate-200 text-slate-700'
                        : 'bg-slate-900/60 hover:bg-slate-900 border-slate-800 text-slate-200 hover:text-white'
          ...
          <div className={`flex-1 min-h-0 relative overflow-hidden rounded-2xl transition duration-300 ${
            ...
            <div className="absolute inset-0 z-10 overflow-y-auto p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6">
              {!activePartner ? (
