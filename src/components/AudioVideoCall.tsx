...
export default function AudioVideoCall({
  ...
  const processSignal = (signal: any, pc: RTCPeerConnection) => {
    signalProcessingQueue.current = signalProcessingQueue.current.then(async () => {
      try {
        if (signal.sdp) {
  ...
  useEffect(() => {
    ...
    async function initCall() {
      try {
        const constraints = {
          audio: true,
          video: callType === 'video' ? { facingMode: { ideal: facingMode } } : false
        };
  ...
  // Connection state is handled directly in ontrack and connection state change listeners.
  ...
  return (
    ...
    >
      ...
      {callStatus === 'Connection Lost' && (
        <div className="absolute inset-0 bg-slate-950/85 backdrop-blur-md z-30 flex flex-col items-center justify-center p-6 sm:p-8 text-center animate-fade-in text-slate-100">
          ...
          <div className="max-w-xs space-y-3 mx-auto mb-5">
          ...
          <button
            ...
            className="mt-2 inline-flex items-center justify-center rounded-full bg-rose-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-rose-600/30 hover:bg-rose-500 transition focus:outline-none focus:ring-2 focus:ring-rose-500/50"
          >
            Cancel Call
          </button>
        </div>
      ...
      <div className="flex-1 w-full relative min-h-0">
        {callType === 'video' ? (
          <div className="absolute inset-0 w-full h-full bg-slate-900">
            ...
            <div className="w-full h-full relative flex items-center justify-center bg-black">
              ...
              {/* Note: User Name label moved inside relative container, removed bottom absolute positioning to avoid overlap */}
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                <div className="bg-slate-950/80 rounded-3xl border border-slate-800 px-4 py-3 text-left max-w-[calc(100vw-8rem)]">
            ...
            {!isVideoOff && !errorMsg && (
              <div className="absolute top-4 right-4 w-28 sm:w-48 aspect-[3/4] sm:aspect-video rounded-xl overflow-hidden border border-slate-700 shadow-2xl bg-slate-950 z-10">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center w-full h-full bg-gradient-to-tr from-slate-950 via-slate-900 to-indigo-950/20">
          <audio
            ref={remoteAudioRef}
            autoPlay
            className="hidden"
          ...
          <p className="text-indigo-400 text-xs tracking-wider uppercase font-semibold animate-pulse mb-2">
          ...
          {!isCallConnected && (
            <div className="w-16 h-1 bg-slate-800 rounded-full my-6 flex overflow-hidden mx-auto">
               <span className="h-full bg-violet-500 animate-slide-wide w-1/2"></span>
            </div>
          )}
          <span className="text-slate-500 text-[11px] mt-4 block">Secure end-to-end encrypted connection</span>
        </div>
      )}
      </div>

      {/* INTERACTIVE CONTROLS BAR */}
      <div className="w-full flex-shrink-0 bg-slate-950 border-t border-slate-800 p-3 sm:p-4 z-20">
        <div className="grid gap-2 grid-cols-[repeat(auto-fit,minmax(64px,1fr))] sm:flex sm:flex-wrap sm:items-center sm:justify-center sm:gap-3 max-w-4xl mx-auto lg:flex lg:flex-row lg:items-center lg:justify-center lg:space-x-3">
        {/* Toggle Audio Mute */}
        <button
          onClick={toggleMute}
        ...
        <button
          ...
          className="p-3 sm:p-3.5 bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 text-slate-300 hover:text-white rounded-full sm:rounded-xl transition duration-200 cursor-pointer hidden sm:inline-flex"
        >
