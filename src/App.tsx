...
export default function App() {
  ...
  const [sidebarTab, setSidebarTab] = useState<'people' | 'chat' | 'vip'>('people');
  ...
  const screenRef = useRef<'lobby' | 'matching' | 'plans' | 'geo' | 'admin'>('lobby');
  ...
  const [screen, setScreen] = useState<'lobby' | 'matching' | 'plans' | 'geo' | 'admin'>('lobby');
  ...
  useEffect(() => {
    ...
    const handleAndroidBack = (evt: Event) => {
      ...
      if (!isLoggedInRef.current) {
        ...
        window.sessionStorage.setItem('vibe_allow_quit', 'true');
        window.history.go(-2);
        return;
      }
  ...
  const connectWebSocket = (userId: string, tokenOverride?: string) => {
    ...
    const wsUrl = `${protocol}//${window.location.host}/api/ws?token=${wsToken}`;
    ...
    socket.onmessage = (e) => {
      try {
        ...
        if (event === 'welcome') {
          ...
          const prevOrig = outgoingCallRef.current;
  ...
  const handleInitiateCall = (recipientId: string, type: 'audio' | 'video', recipientName?: string, recipientPic?: string) => {
    if (!ws || !me) return;
    ...
    setOutgoingCall({
      ...
      targetName: recipientName || 'Connecting...', 
      targetPic: recipientPic || '',
      type
    });
  };
  ...
  const handleHangupCall = () => {
    ...
    const event = new CustomEvent('app:call_ended', { detail: {} });
  ...
  return (
    <div id="app-root-container" className={`fixed inset-0 w-full max-w-[100vw] overflow-hidden font-sans antialiased flex flex-col justify-between theme-transition ${
      theme === 'light' 
      ...
      {toast && (
        <div className={`${(activeCall || incomingCall || outgoingCall) ? 'fixed inset-0 z-50 flex items-center justify-center pointer-events-none' : 'fixed top-6 right-6 z-50 pointer-events-none'} p-4 animate-fade-in`}> 
          <div className={`mx-auto rounded-2xl border px-4 py-3 text-xs font-semibold shadow-2xl flex items-center gap-2 max-w-sm ${
            toast.isError
              ? 'bg-rose-500/90 border-rose-500 text-white shadow-rose-500/20'
              : 'bg-indigo-500/90 border-indigo-500 text-white shadow-indigo-500/20'
          }`}>
      ...
      {!activeCall && messageBanner && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 p-4 animate-fade-in pointer-events-none">
          <div className={`mx-auto rounded-2xl border px-4 py-3 text-sm font-semibold shadow-2xl flex items-center gap-3 max-w-sm cursor-pointer pointer-events-auto transition hover:scale-105 ${
            theme === 'light' ? 'bg-white border-slate-200 text-slate-800' : 'bg-slate-900 border-slate-700 text-slate-100'
          }`} onClick={() => { setScreen('lobby'); setSidebarTab('chat'); setMessageBanner(null); }}>
            <div className="flex items-center justify-center w-8 h-8 rounded-full bg-violet-600/20 text-violet-500 shrink-0">
              <MessageSquare className="w-4 h-4" />
            </div>
            <div className="flex flex-col min-w-0">
              <span className="text-xs text-slate-500 dark:text-slate-400 font-bold tracking-wider uppercase mb-0.5">New Message</span>
              <span className="truncate max-w-[200px]">{messageBanner.text}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {activeCall && (
        <div className="modal-overlay call-modal-overlay bg-[#070B16] z-50 pointer-events-auto">
      ...
      {incomingCall && (
        <div className="modal-overlay bg-slate-950/80 backdrop-blur-sm z-50 animate-fade-in pointer-events-auto">
                  ...
                  <>
                    <div className="flex-1 min-h-0 flex flex-col w-full">
                      <div className="flex-1 min-h-0 relative">
                        <div className={`absolute inset-0 transition-opacity duration-300 ${(screen === 'plans' || screen === 'matching') ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                          <ChatInterface
                            ws={ws}
                            me={me}
                            token={token}
                            ...
                            sidebarTab={sidebarTab as any}
                            setSidebarTab={setSidebarTab as any}
                            ...
                            onNewMessageNotification={(text) => {
                              const bannerId = Date.now();
                              setMessageBanner({ text, id: bannerId });
                              setTimeout(() => setMessageBanner(prev => prev?.id === bannerId ? null : prev), 3500);
                            }}
                            isAppOverlayOpen={
                              showOwnProfileModal ||
                              showNotificationsDropdown ||
                        ...
                        {screen === 'matching' && (
                          <div className="absolute inset-0 z-30">
                            <MatchingRoom
                              ws={ws}
                              me={me}
                              onExit={() => { setScreen('lobby'); setSidebarTab('people'); }}
                              onTriggerVipPage={triggerVipPage}
                            />
                          </div>
                        )}
                      ...
                      {(screen === 'lobby' || screen === 'plans' || screen === 'matching') && (
                        <nav className={`shrink-0 w-full max-w-full flex justify-between gap-1 p-1.5 sm:p-3 border-t transition duration-300 overflow-hidden ${isChatActiveMobile ? 'hidden md:flex' : ''} ${
                          ...
                          <button
                            onClick={() => { setScreen('matching'); setShowNotificationsDropdown(false); setShowOwnProfileModal(false); }}
